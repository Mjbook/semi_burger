...
public class SideDAO {
	...
	public ArrayList<SideDTO> SideMenu(SideDTO sdto){
		try{
			...
			while(rs.next()){
				...
				SideDTO sidedto= new SideDTO(sideitem_key, sideitem_name, sideitem_pay, sideitem_count, sideitem_event_start, sideitem_event_finish, sideitem_img_src);
	...
	/**메뉴수정 메서드*/
	public int sideReLoad(int sideitem_key,String sideitem_name,String sideitem_pay,int sideitem_count,String sideitem_img_src){
		try{
			conn=yb.db.YB_DB.getConn();
			String sql="update side set sideitem_name=?,sideitem_pay=?,sideitem_count=?,sideitem_img_src=? where sideitem_key=?";
			ps=conn.prepareStatement(sql);
			ps.setString(1, sideitem_name);
			ps.setString(2, sideitem_pay);
			ps.setInt(3, sideitem_count);
			ps.setString(4, sideitem_img_src);
			ps.setInt(5, sideitem_key);
			int count=ps.executeUpdate();
	...
	/**메뉴삭제 메서드*/
	public int sideDelete(int sideitem_key){
		try{
			conn=yb.db.YB_DB.getConn();
			String sql="delete from side where sideitem_key=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, sideitem_key);
			int count=ps.executeUpdate();
